...
function dumbState() {
    ...
    var enoughHealthyBorg = weaponTeam.filter(i => i.propulsion == "CyborgLegs" && i.health > 60).length > healthyBorgRequirement * weaponTeam.filter(i => i.propulsion == "CyborgLegs").length
...
function dumbTank() {
    ...
    if (state == 0) weaponTeam.forEach((droid, i) => {
        ...
        var odd = calculateTankOdd(droid.id)
    ...
    if (state == 1) weaponTeam.forEach((droid, i) => {
        ...
        //recycle old droid
        if (lr < 5 && droid.born < gameTime * 1 / 2 && countDroid(DROID_ANY, me) > getDroidLimit(me) - 10) {
            lr += 1
...
function calculateTankOdd(droid_id) {
    return (droid_id % oddGroups - (oddGroups - 1) / 2) / ((oddGroups - 1) / 2) //normalized to [-1, 1]
}
